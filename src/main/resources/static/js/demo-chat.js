var demoChat = {
    comms : {
        ...
        connect: function(rcvMessage, rcvEvent, email) {
           ...
           demoChat.comms.stompClient.connect({'X-Email':email}, function(frame) {
    ...
    gui : {
        ...
        login: function(){
            ...
            var gravatar = demoChat.gui.getGravatar(demoChat.gui.email);
            $("#login").hide();
            $("#chat").show();
            $("#avatar").attr('src', gravatar);
            demoChat.comms.connect(demoChat.gui.receiveMessage, demoChat.gui.receiveEvent, demoChat.gui.email);
        },
        ...
        receiveEvent: function(event) {
            ...
            if (jsonEvent.type == "CONNECT") {
                demoChat.gui.addParticipant(jsonEvent.email);
            } else {
                demoChat.gui.removeParticipant(jsonEvent.email);
            }

        },

        addParticipant: function(email){
            var gravatar = demoChat.gui.getGravatar(email, 40);
